...
namespace SharedComponents.Utility
{
    ...
    public class Util
    {
        ...
        public static string FindNativeModulePath(string dllName)
        {
            ...
            foreach (ProcessModule module in process.Modules)
            {
                ...
                {
                    if (GetModuleFileNameEx(hProcess, module.BaseAddress, filePath, (uint)filePath.Capacity) > 0)
                    {
        ...
        public static long HashLong(long value)
        {
            unchecked
            {
                value = (~value) + (value << 21); // First transformation
                value = value ^ (value >> 24);    // Mix bits
                value = (value + (value << 3)) + (value << 8); // More mixing
                value = value ^ (value >> 14);    // Spread bits
                value = (value + (value << 2)) + (value << 4); // Another round
                value = value ^ (value >> 28);    // Final mix
                value = value + (value << 31);    // Spread high bits
                return value;
            }
        }
        ...
        {
            // The OSVersionInfoSize field must be set to PyMarshal.SizeOf(typeof(OSVERSIONINFOEX))
